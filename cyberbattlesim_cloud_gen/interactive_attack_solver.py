...
class InteractiveAttackPathSolver:
    ...
    def _apply_action(self, action: Tuple) -> Tuple[AttackState, float]:
        ...
        if action_type == "change_source":
            ...
            cost = vuln_info.cost
            new_state = self.current_state.apply_outcome(
                target_node, vuln_info.outcome, self.nodes
            )
    ...
    def execute_action(self, action_id: str) -> Dict:
        ...
        progress = {
            ...
            "progress_made": (
                ...
                or known_after > known_before
                or creds_after > creds_before
            ),
        }
    ...
    def undo(self) -> Dict:
        ...
        self.current_state = AttackState(
            ...
            credentials=frozenset(
                tuple(c) if isinstance(c, list) else c for c in prev["credentials"]
            ),
        )
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "completion_percentage": (owned / self.total_nodes * 100)
        ...
        for node_id in self.nodes:
            for other_id in self.nodes:
                if other_id != node_id:
                    ...
                    if key not in edge_set and self._check_firewall_access(
                        node_id, other_id, "*"
                    ):
...
def create_flask_app(
    solver: InteractiveAttackPathSolver, html_path: str = None
) -> Flask:
...
def run_interactive_server(
    ...
    print("API endpoints:")
    print("  GET  /api/export  - Get full state")
    print("  GET  /api/status  - Get current status")
    print("  GET  /api/actions - Get available actions")
    print("  POST /api/action/<id> - Execute action")
    print("  POST /api/undo    - Undo last action")
