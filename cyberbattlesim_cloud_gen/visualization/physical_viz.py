...
def load_physical_topology(filepath: str) -> Dict:
    ...
    try:
        ...
        if "physical_topology" not in cluster_config:
            print(
                f"Error: 'physical_topology' key not found in {filepath}",
                file=sys.stderr,
            )
            return {}
...
def build_node_network_graph(topology: Dict) -> nx.DiGraph:
    ...
    if "node_connections" not in topology:
        print(
            "Warning: No 'node_connections' found. Graph will have no edges.",
            file=sys.stderr,
        )
        return G
...
def visualize_node_network(G: nx.DiGraph, output_file: str, color_by: str = "type"):
    ...
    plt.title(
        f"Physical Node Network Topology (Colored by {color_by})",
        fontsize=20,
        fontweight="bold",
    )
    plt.axis("off")
...
def visualize_node_service_mapping(G: nx.Graph, output_file: str, topology: Dict):
    ...
    nx.draw_networkx_labels(
        G, pos, labels=labels, font_size=8, font_weight="bold", font_family="monospace"
    )
    nx.draw_networkx_edges(
        G, pos, width=0.5, alpha=0.3, edge_color="#495057", arrows=False
    )
...
def visualize_physical_from_data(cluster_data: Dict, output_base_path: str):
    ...
    visualize_node_network(
        node_network_graph, f"{base_output}_nodes_by_type.png", color_by="type"
    )
    visualize_node_network(
        node_network_graph, f"{base_output}_nodes_by_zone.png", color_by="zone"
    )
    visualize_node_service_mapping(
        node_service_graph, f"{base_output}_mapping.png", topology
    )
...
if __name__ == "__main__":
    ...
    if topology:
        ...
        visualize_node_network(
            node_network_graph, f"{output_base}_nodes_by_type.png", color_by="type"
        )
        visualize_node_network(
            node_network_graph, f"{output_base}_nodes_by_zone.png", color_by="zone"
        )
        visualize_node_service_mapping(
            node_service_graph, f"{output_base}_mapping.png", topology
        )
