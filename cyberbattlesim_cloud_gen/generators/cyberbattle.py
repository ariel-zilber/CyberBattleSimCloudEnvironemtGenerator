...
class ClusterAttackGenerator(NetworkGenerator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if cluster_config:
            ...
            print("✅ Using in-memory cluster configuration")
        elif config_file:
            try:
                with open(config_file, "r") as f:
                    self.cluster_config = json.load(f)
                print(
                    f"✅ Successfully loaded cluster configuration from {config_file}"
                )
            except Exception as e:
                print(
                    f"❌ Error: Could not load or parse config file {config_file}. {e}"
                )
                raise
        else:
    ...
    def _merge_vulnerability(
        self, existing_vulns: Dict, new_vuln_id: str, new_vuln: VulnerabilityInfo
    ):
        """
        ...
        if isinstance(existing_vuln.outcome, LeakedCredentials) and isinstance(
            new_vuln.outcome, LeakedCredentials
        ):
            try:
                ...
                combined = list(existing_vuln.outcome.credentials) + list(
                    new_vuln.outcome.credentials
                )
                existing_vuln.outcome = LeakedCredentials(credentials=combined)

        # 2. Merge LeakedNodesId
        elif isinstance(existing_vuln.outcome, LeakedNodesId) and isinstance(
            new_vuln.outcome, LeakedNodesId
        ):
    ...
    def _get_library_vulnerabilities(
        self, service_name: str, node_id: str, port: int, credential_id: str
        ...
        scoring_strategy = (
            self.config_loader_instance.get_vulnerability_scoring_strategy()
        )
        ...
        for cve in cve_list:
            ...
            if "TEMP" in cve_id or "Unknown" in cve_id:
                continue
            ...
            if outcome_str == "LeakedCredentials":
                outcome_params["credentials"] = [
                    CachedCredential(
                        node=node_id, port=str(port), credential=credential_id
                    )
                ]
            ...
            cost = (
                base_score - (sev_weight * sev_mult) - (norm_exploitability * exp_mult)
            )
    ...
    def _create_cyberbattle_node(self, physical_node: Dict) -> NodeInfo:
        ...
        for pod_assignment in physical_node["assigned_pods"]:
            ...
            if not isinstance(port_val, int):
                try:
                    ...
                    raise ValueError(f"Invalid port for {service_name}")
            ...
            if not any(s.name == service_name for s in services):
                service = ListeningService(
                    name=service_name, port=port_str, allowedCredentials=[credential]
                )
                services.append(service)

            # 4. Inject Vulnerabilities (Library)
            library_vulns = self._get_library_vulnerabilities(
                service_name, node_id, port_val, credential
            )
            ...
            if "network_topology" in self.cluster_config:
                network_services = self.cluster_config["network_topology"].get(
                    "services", {}
                )
                if (
                    service_name in network_services
                    and "vulnerabilities" in network_services[service_name]
                ):
                    for cve in network_services[service_name]["vulnerabilities"]:
                        ...
                        if outcome_type == "LeakedNodesId":
                            ...
                            if target_nodes:
                                kwargs["credentials"] = [
                                    CachedCredential(
                                        node=target_nodes[0],
                                        port=str(target_port_val),
                                        credential=credential_name,
                                    )
                                ]

                        outcome = self._map_outcome_string_to_class(
                            outcome_type, **kwargs
                        )

                        v_type_str = cve.get(
                            "vulnerability_type", cve.get("type", "REMOTE")
                        ).upper()
                        v_type = (
                            VulnerabilityType.LOCAL
                            if v_type_str == "LOCAL"
                            else VulnerabilityType.REMOTE
                        )
                        ...
                        self._merge_vulnerability(
                            vulnerabilities, cve_vuln_name, new_vuln
                        )
        ...
        if node_value == 0:
            is_attacker_node = (
                physical_node.get("node_type") in ["internet", "external"]
                or "attacker" in physical_node.get("properties", [])
                or node_id == "attacker"
                or "internet" in node_id
            )
    ...
    def _find_related_nodes(self, service_name: str) -> List[str]:
        ...
        if "network_topology" in self.cluster_config:
            access_connectivity = self.cluster_config["network_topology"].get(
                "access_connectivity", []
            )
    ...
    def _create_firewall_rules(self, physical_node: Dict) -> FirewallConfiguration:
        ...
        if "network_topology" in self.cluster_config:
            ...
            for pod in physical_node["assigned_pods"]:
                ...
                if svc_name in net_services and net_services[svc_name].get(
                    "is_public", False
                ):
                    is_public_node = True
            ...
            for port in public_ports:
                incoming_rules.append(
                    FirewallRule(
                        ...
                        reason="Public access for service",
                    )
                )
            ...
            for conn in self.cluster_config["physical_topology"]["node_connections"]:
                if conn["to"] == node_id:
                    permission = (
                        ...
                        if not conn["is_firewalled"]
                        else RulePermission.BLOCK
                    )
                    incoming_rules.append(
                        FirewallRule(
                            port="*",
        ...
        incoming_rules.append(
            FirewallRule(
                port="*", permission=RulePermission.BLOCK, reason="Default deny"
            )
        )
        outgoing_rules.append(
            FirewallRule(
                port="*", permission=RulePermission.ALLOW, reason="Default allow"
            )
        )
        ...
        permissive_firewall = FirewallConfiguration(
            incoming=[
                FirewallRule(
                    port="*",
                    permission=RulePermission.ALLOW,
                    subnet=Subnet("0.0.0.0/0"),
                    reason="Attacker Full Access",
                )
            ],
            outgoing=[
                FirewallRule(
    ...
    def _find_public_nodes(self) -> List[str]:
        ...
        if "network_topology" in self.cluster_config:
            for service_name, service_config in (
                self.cluster_config["network_topology"].get("services", {}).items()
            ):
                if (
                    service_config.get("is_public", False)
                    or service_name in public_services
                ):
    ...
    def _build_credential_flows(self) -> None:
        if "network_topology" not in self.cluster_config:
            return
        credential_flows = self.cluster_config["network_topology"].get(
            "credential_flow", []
        )
    ...
    def _generate_analysis_report(self, attack_paths: Dict, analysis_dir: str):
        ...
        with open(report_path, "w") as f:
            ...
            total_nodes = len(
                [n for n in self.nodes if "attacker" not in self.nodes[n].properties]
            )
            f.write(f"BASIC STATISTICS:\nTotal target nodes: {total_nodes}\n")
            f.write(
                f"Total vulnerabilities: {sum(len(node.vulnerabilities) for node in self.nodes.values())}\n"
            )
            f.write(
                f"Attacker entry points: {len([n for n in self.nodes if 'attacker' in self.nodes[n].properties])}\n\n"
            )
            ...
            if "maximal_achievement" in attack_paths:
                ...
                f.write(
                    f"Maximal achievement: {optimal['nodes_owned']}/{total_nodes} nodes owned\nSteps: {len(optimal['path'])}\n\nATTACK PATH:\n"
                )
            ...
            critical_nodes = [
                ...
                if "attacker" not in n.properties and n.value >= 100
            ]
            for nid, val in sorted(critical_nodes, key=lambda x: x[1], reverse=True):
                f.write(f"{nid}: value {val}\n")
    ...
    def get_nodes(self) -> Dict[str, NodeInfo]:
        ...
        if "physical_topology" in self.cluster_config:
            for physical_node in self.cluster_config["physical_topology"]["nodes"]:
                if not physical_node["is_healthy"]:
                    continue
                if (
                    physical_node["node_type"] == "internet"
                    or physical_node["node_id"] == "node-internet"
                ):
                    print(
                        "   ✓ Found existing Internet node, converting to Attacker entry point."
                    )
                    ...
                    public_nodes = self._find_public_nodes()
                    cyber_node.vulnerabilities["InitialScan"] = VulnerabilityInfo(
                        description="Initial network reconnaissance",
                        type=VulnerabilityType.LOCAL,
                        outcome=LeakedNodesId(nodes=list(public_nodes)),
                        cost=1.0,
                    ...
                    cyber_node.vulnerabilities["PortScan"] = VulnerabilityInfo(
                        description="Detailed port scanning",
                        type=VulnerabilityType.LOCAL,
                        outcome=LeakedNodesId(nodes=list(public_nodes)),
                        cost=2.0,
                    ...
                    for prop in ["attacker", "external", "reconnaissance"]:
                        if prop not in cyber_node.properties:
                            cyber_node.properties.append(prop)
                    self.nodes["attacker"] = cyber_node
                else:
                    cyber_node = self._create_cyberbattle_node(physical_node)
                    self.nodes[physical_node["node_id"]] = cyber_node

        if not internet_node_found:
    ...
    def generate(self):
        ...
        attacker_nodes = sum(
            1 for node in self.nodes.values() if "attacker" in node.properties
        )
        print(
        ...
        if "maximal_achievement" in attack_paths:
            optimal = attack_paths["maximal_achievement"]
            print(
                f"   ✅ Maximal achievement: {optimal['nodes_owned']}/{len(self.nodes) - attacker_nodes} nodes owned\n   ⚡ Attack steps: {len(optimal['path'])}"
            )
...
def main():
    ...
    parser.add_argument(
        "out_dir", type=str, help="Directory to write the generated environment files"
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
