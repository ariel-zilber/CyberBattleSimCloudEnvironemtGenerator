...
from cyberbattlesim_cloud_gen.generators.physical_gen.network_access_graph import NetworkAccessGraph
from cyberbattlesim_cloud_gen.generators.physical_gen.node_characteristics import NodeCharacteristics
from cyberbattlesim_cloud_gen.generators.physical_gen.node_connection import NodeConnection
...
from cyberbattlesim_cloud_gen.generators.physical_gen.security_group import SecurityGroup
...
class PhysicalNodeGenerator:
    ...
    def __init__(self, cluster_config: Dict, num_nodes: Optional[int] = None, 
                 ...
                 seed: Optional[int] = None, config_loader_instance: ConfigLoader = None):
    ...
    def _auto_determine_zones(self) -> int:
        ...
        if self.cluster_size == ClusterSize.TINY: return 1
        elif self.cluster_size == ClusterSize.SMALL: return 2
        elif self.cluster_size == ClusterSize.MEDIUM: return 3
        elif self.cluster_size == ClusterSize.LARGE: return 4
        else: return 6
    ...
    def _initialize_network_subnets(self):
        ...
        if self.num_zones <= 256: subnet_prefix = 16
        else: subnet_prefix = 24
        ...
        for i, zone in enumerate(available_zones):
            if i < len(zone_subnets):
                ...
                self.zone_subnets[zone] = ipaddress.ip_network(f"10.{i}.0.0/{subnet_prefix}")
    ...
    def _allocate_ip_for_node(self, zone: str, node_index: int) -> IPAllocation:
        ...
        if zone == "public":
            return IPAllocation(ipv4="0.0.0.0", subnet="0.0.0.0", gateway="0.0.0.0", cidr="0.0.0.0/0")
        ...
        return IPAllocation(ipv4=ip_address, subnet=str(subnet.netmask), gateway=gateway, cidr=cidr)
                ...
                for other_zone, other_subnet in self.zone_subnets.items():
                    if other_zone != zone:
                        rules.append(SecurityGroupRule(
                            ...
                            destination_cidr=self.zone_subnets[zone].with_prefixlen,
                            action="allow", description=f"Allow traffic from {other_zone}"
                        ))
            
            self.security_groups[zone] = SecurityGroup(group_id=group_id, zone=zone, name=f"security-group-{zone}", rules=rules)
    ...
    def _generate_resource_value(self, config: Union[Dict, int, float, List]) -> Union[int, float]:
        """
        ...
        if isinstance(config, list):
            if len(config) == 2:
                return random.choice([config[0], (config[0]+config[1])//2, config[1]])
    ...
    def _calculate_node_asset_value(self, node_type: str, services: List[str]) -> int:
        ...
        type_mult = strategy.get('multipliers', {}).get('node_type', {}).get(node_type, 1.0)
        current_value = base_value * type_mult
        
        # Get multiplier dictionaries
        service_cat_mults = strategy.get('multipliers', {}).get('services', {})
        specific_svc_mults = strategy.get('multipliers', {}).get('specific_services', {})
    ...
    def _assign_zone(self, node_index: int, available_zones: List[str]) -> str:
        if not available_zones: return "zone-a"
    ...
    def _get_node_affinity(self, service_name: str) -> List:
        ...
        return service_node_affinity.get(service_name, service_node_affinity.get("default", []))
    ...
    def _assign_pods_to_nodes(self):
        ...
        for service_name, instance_id in pods_to_assign:
            ...
            if not eligible_nodes:
                try: eligible_nodes = [n for n in self.nodes if n.node_type == NodeType.WORKER]
                except AttributeError: pass
            
            if not eligible_nodes:
                try: eligible_nodes = [n for n in self.nodes if n.node_type != NodeType.INTERNET]
                except AttributeError: eligible_nodes = self.nodes
        ...
        for i, node1 in enumerate(self.nodes):
            for node2 in self.nodes[i+1:]:
                connections.append(NodeConnection(
                    from_node=node1.node_id, to_node=node2.node_id,
                    from_ip=node1.ip_allocation.ipv4, to_ip=node2.ip_allocation.ipv4,
                    latency_ms=random.uniform(0.1, 2.0), bandwidth_gbps=10.0, packet_loss=0.0
                ))
                connections.append(NodeConnection(
                    from_node=node2.node_id, to_node=node1.node_id,
                    from_ip=node2.ip_allocation.ipv4, to_ip=node1.ip_allocation.ipv4,
                    latency_ms=random.uniform(0.1, 2.0), bandwidth_gbps=10.0, packet_loss=0.0
                ))
        return connections
    ...
    def _generate_service_mapping(self) -> Dict[str, List[str]]:
        ...
        for node in self.nodes:
            for service_name, instance_id in node.assigned_pods:
                ...
                if service_name not in mapping: mapping[service_name] = []
                mapping[service_name].append({
                    "pod_id": pod_id, "node_id": node.node_id, "zone": node.zone,
                    "node_ip": node.ip_allocation.ipv4, "services_ports": service_ports.get(service_name, [])
                })
        return mapping
    ...
    def _apply_realistic_firewall_rules(self, permissive_level: float):
        ...
        public_services = set(self.cluster_config.get("public_services", []) + list(public_services))
        all_cidrs = list([str(subnet) for subnet in self.zone_subnets.values()])
        dmz_zone = "zone-a"
        
        for node in self.nodes:
            try:
                if node.node_type == NodeType.INTERNET: continue
            except AttributeError: pass
                
            incoming, outgoing = [], []
            
            for cidr in all_cidrs:
                outgoing.append({"subnet": cidr, "permission": "ALLOW", "port": "*", "reason": "Internal Routing"})
                if node.zone != "public":
                    incoming.append({"subnet": cidr, "permission": "ALLOW", "port": "*", "reason": "Internal Trust"})
            ...
            if is_public_node and node.zone == dmz_zone:
                ...
                for port in ports:
                    incoming.append({"subnet": "0.0.0.0/0", "permission": "ALLOW", "port": str(port), "reason": "Public Service Access"})
            
            if permissive_level > 0 and random.random() < permissive_level:
                incoming.append({"subnet": "0.0.0.0/0", "permission": "ALLOW", "port": "22", "reason": "MISCONFIG: SSH Exposed"})
                
            incoming.append({"subnet": "0.0.0.0/0", "permission": "DENY", "port": "*", "reason": "Default Deny"})
            outgoing.append({"subnet": "0.0.0.0/0", "permission": "DENY", "port": "*", "reason": "Default Deny"})
            node.firewall = {"incoming": incoming, "outgoing": outgoing}

    def generate(self, firewall_probability: float = 0.1, firewall_cross_zone_only: bool = False,
                node_failure_probability: float = 0.05, permissive_level: float = 0.1) -> Dict:
        ...
        try:
            ...
            internet_node.firewall = {
                "incoming": [{"subnet": "0.0.0.0/0", "permission": "ALLOW", "port": "*", "reason": "Internet"}],
                "outgoing": [{"subnet": "0.0.0.0/0", "permission": "ALLOW", "port": "*", "reason": "Internet"}]
            }
        ...
        topology = {
            ...
            "network_configuration": {
                ...
                "ip_allocations": {zone: list(ips) for zone, ips in self.zone_ip_allocations.items()}
            },
