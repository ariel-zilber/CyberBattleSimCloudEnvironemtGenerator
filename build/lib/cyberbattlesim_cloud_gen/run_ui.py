...
try:
    from cyberbattlesim_cloud_gen.interactive_attack_solver import (
    ...
    print(
        "Please ensure you are in the root directory of the project and dependencies are installed."
    )
    sys.exit(1)
...
def load_scenario(nodes_dir: str) -> Dict[str, NodeInfo]:
    ...
    files = [
        f for f in os.listdir(nodes_dir) if f.endswith(".yaml") or f.endswith(".yml")
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Interactive CyberBattleSim Demo from Existing Scenario"
    )
    parser.add_argument(
        "--scenario-dir",
        type=str,
    ...
    for n in nodes:
        print(
            f"  ✓ Loaded node: {n} with {len(nodes[n].vulnerabilities)} vulnerabilities and {len(nodes[n].services)} services"
        )
    if not nodes:
    ...
    run_interactive_server(
        nodes, attacker_node, port=args.port, html_path=args.html_file
    )
