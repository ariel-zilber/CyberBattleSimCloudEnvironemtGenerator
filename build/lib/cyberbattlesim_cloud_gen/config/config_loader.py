...
class ConfigLoader:
    ...
    def _create_node_type_enum(self):
        ...
        # Add Aliases
        if "compute" in self.node_profiles_data:
            enum_members["COMPUTE_OPTIMIZED"] = "compute"
        if "memory" in self.node_profiles_data:
            enum_members["MEMORY_OPTIMIZED"] = "memory"
        if "storage" in self.node_profiles_data:
            enum_members["STORAGE_OPTIMIZED"] = "storage"
    ...
    def get_node_profile_config(self, node_type_name: str) -> Dict[str, Any]:
        ...
        key = (
            ...
            if hasattr(node_type_name, "value")
            else str(node_type_name)
        )
        # Fallback to 'worker' if type not found to prevent crashes
        return self.node_profiles_data.get(key.lower()) or self.node_profiles_data.get(
            "worker", {}
        )
    ...
    def get_address_space_config(self) -> Dict[str, Any]:
        ...
        return self.topology_data.get(
            "address_space", {"base_network": "10.0.0.0/8", "subnet_prefix": 24}
        )
    ...
    def get_public_services(self) -> Set[str]:
        return {
            name
            for name, cfg in self.services.items()
            if "public" in cfg.get("tags", []) or cfg.get("is_public", False)
        }
    ...
    def get_control_plane_services(self) -> List[str]:
        return [
            name
            for name, cfg in self.services.items()
            if "control_plane" in cfg.get("tags", [])
        ]
    ...
    def get_connectivity_rules(self) -> Dict[str, Any]:
        ...
        for name, cfg in self.services.items():
            ...
            rules[name] = {
                ...
                "connectivity_probability": conn_cfg.get(
                    "probability", self.defaults["connectivity_probability"]
                ),
                "port_range": port_range,
                "requires_auth": conn_cfg.get(
                    "requires_auth", self.defaults["requires_auth"]
                ),
                "is_critical": is_crit,
                "is_public": is_pub,
            }
    ...
    def get_credential_access_patterns(
        self,
    ) -> Dict[str, Dict[str, Tuple[int, str, List[str]]]]:
        patterns = {}
        for service, cfg in self.services.items():
            ...
            for target_svc, details in cred_access.items():
                if isinstance(details, dict):
                    patterns[service][target_svc] = (
                        details.get("level", 1),
                        details.get("type", "password"),
                        details.get("roles", []),
                    )
        return patterns
    ...
    def get_credential_caching_services(self) -> List[str]:
        return [
            name
            for name, cfg in self.services.items()
            if "credential_caching" in cfg.get("tags", [])
        ]
    ...
    def get_service_vulnerability_profiles(self) -> Dict[str, Any]:
        ...
        for svc, data in self.vuln_profiles_data.items():
            ...
            if "base_vulnerability_count" in profile:
                profile["base_vulnerability_count"] = tuple(
                    profile["base_vulnerability_count"]
                )
    ...
    def get_services_for_use_case(
        self, use_case: str, min_probability: float = 0.0
    ) -> Dict[str, float]:
    ...
    def get_scalable_services(self) -> List[str]:
        ...
        return [
            name
            for name, cfg in self.catalog_data.items()
            if cfg.get("scale_with_cluster", False)
        ]
    ...
    def set_default_vulnerability_profile(
    ...
    def set_subnet_bounds(self, max_subnets: int, max_hosts_per_subnet: int) -> None:
        ...
        self.topology_data["subnets"]["bounds"]["max_hosts_per_subnet"] = (
    ...
    def update_service_connectivity(
        self,
        service: str,
    ...
    def update_service_probability(
        self, service: str, use_case: str, probability: float
    ) -> None:
    ...
    def update_service_dependencies(
        self, service: str, dependencies: List[str]
    ) -> None:
    ...
    def get_vulnerability_scoring_strategy(self) -> Dict[str, Any]:
        ...
        defaults = {
            "base_score": 12.0,
            "min_score": 1.0,
            "multipliers": {"severity": 2.0, "exploitability": 2.0},
            "severity_weights": {
                "LOW": 1.0,
                "MEDIUM": 2.0,
                "HIGH": 3.0,
