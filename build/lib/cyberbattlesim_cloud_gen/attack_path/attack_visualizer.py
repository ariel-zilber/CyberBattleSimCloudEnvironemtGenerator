...
# Must be configured before importing pyplot to avoid backend errors
matplotlib.use("Agg")
...
class AttackVisualizer:
    ...
    def visualize_attack_paths(self, nodes: Dict[str, NodeInfo], attack_paths: Dict):
        ...
        G = nx.DiGraph()
        node_ips = {
            nid: n.network_info[0].ip_address
                    ...
                    if rule.permission == RulePermission.ALLOW:
                        try:
                            if str(rule.subnet) in ["*", "0.0.0.0/0"]:
                                G.add_edge(src_id, tgt_id, type="network_access")
                                break
                            if ipaddress.ip_address(tgt_ip) in ipaddress.ip_network(
                                str(rule.subnet), strict=False
                            ):
                                G.add_edge(src_id, tgt_id, type="network_access")
                                break
                        except:
    ...
    def _create_attack_path_diagram(self, G: nx.DiGraph, attack_paths: Dict):
        ...
        if not attack_paths or "maximal_achievement" not in attack_paths:
            return
        ...
        nx.draw_networkx_nodes(
            G, pos, nodelist=non_path, node_color="#f0f0f0", node_size=300, alpha=0.4
        )
        ...
        if len(valid_nodes) > 2:
            ...
            if intermediates:
                nx.draw_networkx_nodes(
                    G, pos, nodelist=intermediates, node_color="#FFD700", node_size=1000
                )
        ...
        plt.title("Optimal Attack Path Sequence", fontsize=20)
        plt.axis("off")
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, "optimal_attack_path.png"), dpi=150)
        plt.close()
        print("   - Saved optimal_attack_path.png")
    ...
    def _create_discovery_tree(self, attack_paths: Dict):
        ...
        nx.draw_networkx_nodes(
            Tree, pos, node_color="#87CEFA", node_size=800, alpha=0.8
        )
        nx.draw_networkx_nodes(
            Tree,
            pos,
        ...
        nx.draw_networkx_edges(
            Tree, pos, edge_color="#555555", arrowstyle="-|>", arrowsize=20
        )

        labels = {n: n.split("_")[-1] if len(n) > 10 else n for n in Tree.nodes()}
        nx.draw_networkx_labels(
            Tree, pos, labels=labels, font_size=8, font_weight="bold"
        )
        ...
        plt.close()
        print("   - Saved discovery_tree.png")
    ...
    def _create_network_overview(self, G: nx.DiGraph, nodes: Dict[str, NodeInfo]):
        ...
        nx.draw_networkx_nodes(
            G, pos, node_color=node_colors, node_size=600, alpha=0.9, edgecolors="white"
        )
        nx.draw_networkx_edges(
            G,
            pos,
    ...
    def _create_property_discovery_chart(self, attack_paths: Dict):
        ...
        if "maximal_achievement" in attack_paths:
            ...
            for action in path_info["path"]:
                if any(k in action for k in ["exploit", "connect", "LateralMove"]):
                    current += 0.5
                if "change_destination" in action:
                    current += 0.1
                owned_count.append(int(current))

            if len(owned_count) > len(steps):
                owned_count = owned_count[: len(steps)]
            elif len(owned_count) < len(steps):
                steps = steps[: len(owned_count)]
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(self.output_dir, "attack_progression.png"), dpi=150
            )
            plt.close()
        ...
        for action in path:
            ...
            if " from " in action and " to " in action:
                ...
                if len(parts) > 1:
                    ...
                    if len(subparts) > 1:
                        src = subparts[0].strip()
                        tgt = subparts[1].split(" ")[0].strip()
                        if not nodes and src:
                            nodes.append(src)
                        if nodes and nodes[-1] != src and src not in nodes:
                            nodes.append(src)
                        if tgt not in nodes:
                            nodes.append(tgt)
            # Change Dest / Connect to Target
                ...
                if "(" in tgt:
                    tgt = tgt.split("(")[0]
                if " " not in tgt and tgt not in nodes:
                    nodes.append(tgt)
            # On Target
            elif " on " in action:
