...
class AttackState:
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            self.current_destination == other.current_destination and
            self.owned_nodes == other.owned_nodes and
            self.known_nodes == other.known_nodes and
    ...
    def has_credential_for_service(self, target_node: str, service_name: str, 
                                    port: str, protocol: Optional[str],
                                    allowed_creds: set) -> bool:
    ...
    def apply_movement(self, new_source: str = None, 
                       new_destination: str = None) -> 'AttackState':
    ...
    def apply_outcome(self, target_node: str, outcome: VulnerabilityOutcome,
                      nodes: Dict[str, NodeInfo]) -> 'AttackState':
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "credentials": [list(c) if isinstance(c, tuple) else c for c in self.credentials]
        }
...
class InteractiveAttackPathSolver:
    ...
    def apply_action(self, action_id: str) -> Tuple[bool, str]:
        ...
        if action.action_type == "change_source":
            self.current_state = self.current_state.apply_movement(new_source=action.target_node)
        elif action.action_type == "change_destination":
            self.current_state = self.current_state.apply_movement(new_destination=action.target_node)
        elif action.vuln_info and action.vuln_info.outcome:
            self.current_state = self.current_state.apply_outcome(
                action.target_node, 
                action.vuln_info.outcome, 
                self.nodes
            )
    ...
    def get_current_status(self) -> Dict:
        ...
        return {
            ...
            "completion_percentage": (owned / total_nodes * 100) if total_nodes > 0 else 0,
            ...
            for other_id in self.nodes:
                if other_id != node_id:
                    ...
                    if edge_key not in edge_set:
                        ...
                        has_connection = self._check_firewall_access(node_id, other_id, "*")
                        if has_connection:
                            edges_data.append({
                                "source": node_id,
                                "target": other_id,
            ...
            vulnerabilities={
                "CVE-2021-44228": VulnerabilityInfo(
                    ...
                    VulnerabilityOutcome(
                        lateral_move=True,
                        credentials_discovered=[{"name": "db_admin", "type": "password"}]
                    ),
                    1.0,
