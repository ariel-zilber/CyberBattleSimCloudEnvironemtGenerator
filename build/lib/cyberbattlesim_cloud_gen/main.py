...
from cyberbattlesim_cloud_gen.generators.logical_gen.k8s_cluster_generator import K8sClusterGenerator
from cyberbattlesim_cloud_gen.generators.logical_gen.cluster_dynamic_config import ClusterDynamicConfig
...
# -----------------------------
from cyberbattlesim_cloud_gen.generators.network_gen.network_topology_generator import NetworkTopologyGenerator
...
from cyberbattlesim_cloud_gen.generators.vulnerability_gen.vulnerability_assigner import VulnerabilityAssigner
...
def run_pipeline(args):
    ...
    topology_type = args.topology_type if args.topology_type else config_loader_instance.get_topology_type()
    
    net_topo_config = NetworkTopologyConfig(
        topology_type=topology_type,
        connectivity_probability=raw_topo_config.get('connectivity_probability', 0.3),
        topology_matrix=raw_topo_config.get('topology_matrix', None) # Loads exact matrix if defined
    )

    # 3. Firewall Configuration
    firewall_data = raw_topo_config.get('firewall', {})
    valid_fw_keys = {'incoming', 'incoming_exceptions', 'outgoing', 'outgoing_exceptions', 'default_block_probability'}
    ...
    logical_gen = K8sClusterGenerator(logical_config, config_loader_instance=config_loader_instance)
    ...
    # 2. Save Identifiers
    save_yaml(generated['identifiers'].to_dict(), os.path.join(args.sim_output_dir, "identifiers", "identifiers.yaml"))
    
    # 3. Save Vulnerability Library
    save_yaml(generated['vulnerability_library'], os.path.join(args.sim_output_dir, "vulnerability_library", "vulnerability_library.yaml"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--topology-type", type=str, default=None,
