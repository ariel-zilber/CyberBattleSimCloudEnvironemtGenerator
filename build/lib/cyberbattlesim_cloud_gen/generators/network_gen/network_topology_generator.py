...
from cyberbattlesim_cloud_gen.generators.network_gen.credential import CredentialGrant, CredentialLevel
from cyberbattlesim_cloud_gen.generators.network_gen.graph_model import NetworkEdge, NetworkTopology, ServiceNode
import numpy as np
...
class NetworkTopologyGenerator:
    ...
    def _apply_firewall_rules(self, edges: List[NetworkEdge], firewall_probability: float) -> List[NetworkEdge]:
        ...
        for edge in edges:
            is_critical = (
                connectivity_rules.get(edge.source, {}).get("is_critical", False) or
                connectivity_rules.get(edge.target, {}).get("is_critical", False)
            )
    ...
    def _generate_credential_flow(self, access_edges: List[NetworkEdge]) -> List[CredentialGrant]:
        credentials = []
        credential_access_patterns = self.config_loader_instance.get_credential_access_patterns()
        shared_credential_patterns = self.config_loader_instance.get_shared_credential_patterns()
        credential_caching_services = self.config_loader_instance.get_credential_caching_services()
        ...
        for service in self.services:
            ...
            for target, (level_val, cred_type, tags) in patterns.items():
                ...
                for actual_target in actual_targets:
                    ...
                    has_access = any(e.source == service and e.target == actual_target and e.firewall_allowed for e in access_edges)
                    ...
                    if has_access or is_operator or is_system:
                        ...
                        credentials.append(CredentialGrant(
                            ...
                            is_cached=(matched_pattern_key in credential_caching_services),
                            is_shared=is_shared,
