...
class EnhancedK8sClusterGenerator:
    ...
    def generate(self) -> Dict:
        ...
        cluster_config = {
            "cluster_metadata": {
                ...
                "total_resource_weight": sum(self.service_instances.values()),
                "resource_utilization": f"{(sum(self.service_instances.values()) / (self.num_nodes * 10)) * 100:.1f}%",
    ...
    def _group_by_category(self) -> Dict[str, List[str]]:
        """Group selected services by category for backwards compatibility"""
        # Simplified categorization based on service name patterns
        by_category = {}
        
        category_keywords = {
            "control_plane_core": ["etcd", "metrics-server", "kube-state"],
            "control_plane_security": ["cert-manager", "vault", "sealed-secrets", "keycloak"],
            "control_plane_networking": ["nginx-ingress", "kong", "cilium", "apisix", "contour", "haproxy"],
            "observability_metrics": ["prometheus", "grafana", "thanos", "victoriametrics"],
            "observability_logging": ["loki", "fluent", "elasticsearch", "kibana", "opensearch"],
            "observability_tracing": ["jaeger", "tempo", "zipkin"],
            "ci_cd": ["argo-cd", "jenkins", "harbor", "flux"],
            "data_sql": ["postgresql", "mysql", "mariadb"],
            "data_nosql": ["mongodb", "cassandra", "scylladb"],
            "data_caching": ["redis", "valkey", "keydb", "memcached"],
            "data_messaging": ["kafka", "zookeeper", "rabbitmq", "nats"],
            "data_storage": ["minio"],
        ...
        for service in self.selected_services:
            ...
            if not categorized:
                if "other" not in by_category:
                    by_category["other"] = []
                by_category["other"].append(service)
        
        return {k: sorted(v) for k, v in sorted(by_category.items())}
    
    def _generate_stats(self) -> Dict:
        """Generate deployment statistics for backwards compatibility"""
        has_monitoring = bool({"prometheus", "grafana"} & self.selected_services)
        has_logging = bool({"grafana-loki", "loki", "elasticsearch", "fluent-bit", "fluentd"} & self.selected_services)
        has_tracing = bool({"jaeger", "grafana-tempo", "tempo", "zipkin"} & self.selected_services)
        has_gitops = bool({"argo-cd", "flux"} & self.selected_services)
        has_service_mesh = bool({"cilium", "istio", "linkerd"} & self.selected_services)
        ...
        return {
            ...
            "data_layer": {
                ...
                "nosql_databases": len([s for s in self.selected_services if s in ["mongodb", "cassandra", "scylladb"]]),
                "caching": bool({"redis", "valkey", "keydb", "memcached"} & self.selected_services),
                "messaging": bool({"kafka", "rabbitmq", "nats"} & self.selected_services)
            }
        }
...
Examples:
  ...
  python k8s_cluster_generator_enhanced.py --config base_config.yaml --count 5
    ...
    # Network topology generation arguments (from original generator)
    parser.add_argument(
        "--generate-topology",
        action="store_true",
    ...
    for i in range(args.count):
        ...
        if args.generate_topology:
            try:
                ...
                print(f"⚠️  Warning: Could not import topology generator: {e}")
                print("    Network topology generation requires network_topology_generator.py")
